...
export default function CodeEditorPage() {
  ...
  const [pendingCloseTabAfterSave, setPendingCloseTabAfterSave] = useState<string | null>(null)
  ...
  const handleSaveAndClose = async (tabName: string) => {
    const tab = tabs.find(t => t.name === tabName)
    if (!tab) return

    if (tab.isNew) {
      setPendingCloseTabAfterSave(tabName)
      setSaveFileName(tabName.replace(/\.ipynb$/, '').replace(/\.py$/, ''))
      setShowSaveModal(true)
      setTabConfirmClose(null)
    } else {
      setPendingCloseTabAfterSave(tabName)
      await handleSaveFileDirectly(tabName)
      closeTab(tabName, true)
                ...
                <div className="flex justify-end gap-3 pt-2">
                  <button
                    ...
                    onClick={() => {
                      ...
                      setShowNewFolderSaveInput(false)
                      if (pendingCloseTabAfterSave) {
                        setTabConfirmClose(pendingCloseTabAfterSave)
                        setPendingCloseTabAfterSave(null)
                      }
                    }}
